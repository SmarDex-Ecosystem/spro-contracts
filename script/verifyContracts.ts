...
function execVerify(address: string, contractName: string, constructorArgs: string, libraries: string[]) {
    let cli = `forge verify-contract ${address} ${contractName} ${constructorArgs} --watch ${etherscanApiKey} ${chain} ${verifierUrl} ${verbose}`;
...
if (etherscanApiKey) etherscanApiKey = `--etherscan-api-key ${etherscanApiKey}`;
verifierUrl = verifierUrl ? `--verifier-url ${verifierUrl}` : '';

const splitChain = broadcastPath.split('/');
const chain = `--chain ${splitChain[splitChain.length - 2]}`;
