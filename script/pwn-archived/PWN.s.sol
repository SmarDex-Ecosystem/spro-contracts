...
//         revert("PWN: selected chain is not set in deployments/sdlatest.json");
//     }
//
...
//             data: abi.encodeWithSelector(IPWNDeployer.deployAndTransferOwnership.selector, salt, owner, bytecode)
//         });
...
//         bool success = GnosisSafeLike(deployment.deployerSafe).execTransaction({
//             to: address(deployment.deployer),
//             data: abi.encodeWithSelector(IPWNDeployer.deploy.selector, salt, bytecode)
//         });
//         require(success, "Deploy failed");
//         return deployment.deployer.computeAddress(salt, keccak256(bytecode));
//     }
...
//             PWNConfig(_deploy({salt: PWNContractDeployerSalt.CONFIG, bytecode: type(PWNConfig).creationCode}));
//
//         vm.stopBroadcast();
//
...
//
//         // - Loan types
//         deployment.simpleLoan = PWNSimpleLoan(
//             _deploy({
...
//
//         // - Proposals
//         deployment.simpleLoanSimpleProposal = PWNSimpleLoanSimpleProposal(
//             _deploy({
...
//         );
//
//         deployment.simpleLoanListProposal = PWNSimpleLoanListProposal(
//             _deploy({
...
//         );
//
//         deployment.simpleLoanFungibleProposal = PWNSimpleLoanFungibleProposal(
//             _deploy({
...
//         );
//
//         deployment.simpleLoanDutchAuctionProposal = PWNSimpleLoanDutchAuctionProposal(
//             _deploy({
...
//             _deploy({salt: PWNContractDeployerSalt.CONFIG, bytecode: type(PWNConfig).creationCode});
//
//         vm.stopBroadcast();
//
//         vm.startBroadcast(initialConfigHelper);
...
//     --broadcast
//     */
//     /// @dev Expecting to have protocol addresses set in the `deployments/sdlatest.json`
...
//             GnosisSafeLike(safe), contract_, abi.encodeWithSignature("acceptOwnership()")
//         );
...
//         require(
//             address(deployment.simpleLoanDutchAuctionProposal) != address(0),
//             "Simple loan dutch auctin proposal not set"
//         );
...
//     --broadcast
//     */
//     /// @dev Expecting to have daoSafe & config addresses set in the `deployments/sdlatest.json`
