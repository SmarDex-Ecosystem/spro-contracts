...
contract SDSimpleLoan is PWNVault, IERC5646, IPWNLoanMetadataProvider {
    ...
    function _checkRefinanceLoanTerms(uint256 loanId, Terms memory loanTerms) private view {
        ...
        if (loan.creditAddress != loanTerms.credit.assetAddress || loanTerms.credit.amount == 0) {
        ...
        if (
            loan.collateral.category != loanTerms.collateral.category
                ...
                || loan.collateral.id != loanTerms.collateral.id || loan.collateral.amount != loanTerms.collateral.amount
        ...
        if (loan.borrower != loanTerms.borrower) {
            revert RefinanceBorrowerMismatch({currentBorrower: loan.borrower, newBorrower: loanTerms.borrower});
        }
    }
    ...
    /**
     * @notice Transfer collateral to Vault, credit to borrower and SDEX to fee sink.
     * @dev The function assumes a prior token approval to a contract address or signed permits & SDEX token approval.
     * @param loanTerms Loan terms struct.
     */
    function _settleNewLoan(Terms memory loanTerms, LenderSpec calldata lenderSpec) private {
        ...
        uint256 feeAmount = getLoanFee(loanTerms.credit.assetAddress, loanTerms.credit.amount);

        // Fees to sink (burned)
        if (feeAmount > 0) {
            MultiToken.Asset memory feeHelper = MultiToken.Asset({
                category: MultiToken.Category.ERC20,
                assetAddress: config.sdex(),
                id: 0,
                amount: feeAmount
            });
            _pushFrom(feeHelper, loanTerms.lender, config.sink());
        }
     ...
     *         The function assumes a prior token approval to a contract address or signed permits & SDEX token approval.
     * @param refinancingLoanId Id of a loan to be refinanced.
     * @param loanTerms Loan terms struct.
     * @param lenderSpec Lender specification struct.
     */
    function _settleLoanRefinance(uint256 refinancingLoanId, Terms memory loanTerms, LenderSpec calldata lenderSpec)
        private
    {
        ...
        uint256 common = Math.min(repaymentAmount, loanTerms.credit.amount);
        uint256 surplus = loanTerms.credit.amount > repaymentAmount ? loanTerms.credit.amount - repaymentAmount : 0;
        uint256 shortage = surplus > 0 ? 0 : repaymentAmount - loanTerms.credit.amount;
        ...
        bool shouldTransferCommon = loanTerms.lender != loanOwner
        ...
        if (lenderSpec.sourceOfFunds != loanTerms.lender) {
            // Withdraw credit asset to the lender first
            creditHelper.amount = (shouldTransferCommon ? common : 0) + surplus;
     ...
     * @param permitData Callers credit permit data.
     */
    function extendLOAN(ExtensionProposal calldata extension, bytes calldata signature, bytes calldata permitData)
        external
    {
        LOAN storage loan = LOANs[extension.loanId];

        // Check that loan is in the right state
        if (loan.status == 0) revert NonExistingLoan();
        if (loan.status == 3) {
            // cannot extend repaid loan
            revert LoanRepaid();
        }
        ...
        if (msg.sender == loanOwner) {
            if (extension.proposer != loan.borrower) {
                ...
                revert InvalidExtensionSigner({allowed: loan.borrower, current: extension.proposer});
            }
        } else if (msg.sender == loan.borrower) {
            if (extension.proposer != loanOwner) {
                // If caller is borrower, proposer must be loan owner
                revert InvalidExtensionSigner({allowed: loanOwner, current: extension.proposer});
            }
        } else {
        ...
        if (extension.duration < MIN_EXTENSION_DURATION) {
            revert InvalidExtensionDuration({duration: extension.duration, limit: MIN_EXTENSION_DURATION});
        }
        if (extension.duration > MAX_EXTENSION_DURATION) {
            revert InvalidExtensionDuration({duration: extension.duration, limit: MAX_EXTENSION_DURATION});
        }
    ...
    function getExtensionHash(ExtensionProposal calldata extension) public view returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                hex"1901",
                DOMAIN_SEPARATOR,
                keccak256(abi.encodePacked(EXTENSION_PROPOSAL_TYPEHASH, abi.encode(extension)))
            )
        );
    }
