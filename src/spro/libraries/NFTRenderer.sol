...
library NFTRenderer {
    function render(ISproTypes.Loan memory loan, uint256 loanId) public view returns (string memory uri_) {
        ...
        bytes memory svg = abi.encodePacked(
            renderBackgroundAndTop(creditSymbol, collateralSymbol),
    ...
    function renderBackgroundAndTop(string memory creditSymbol, string memory collateralSymbol)
        internal
        pure
    ...
    function renderInfobox(
        ...
        uint256 credit,
        uint256 collateral
    ) internal pure returns (string memory infobox) {
        infobox = string.concat(
