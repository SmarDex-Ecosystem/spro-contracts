...
import {
    MultiToken,
    SDSimpleLoanProposalTest,
    SDSimpleLoanProposal_AcceptProposal_Test
} from "test/unit/SDSimpleLoanProposal.t.sol";

abstract contract SDSimpleLoanSimpleProposalTest is SDSimpleLoanProposalTest {
    SDSimpleLoanSimpleProposal proposalContract;
    ...
    function setUp() public virtual override {
        ...
        proposalContract = new SDSimpleLoanSimpleProposal(hub, revokedNonce, config);
        proposalContractAddr = SDSimpleLoanProposal(proposalContract);
    ...
    function _proposalHash(SDSimpleLoanSimpleProposal.Proposal memory _proposal) internal view returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                keccak256(
                    abi.encode(
                        ...
                        keccak256("SDSimpleLoanSimpleProposal"),
                        keccak256("1.0"),
                        block.chainid,
    ...
    function _updateProposal(SDSimpleLoanProposal.ProposalBase memory _proposal) internal {
...
contract SDSimpleLoanSimpleProposal_CreditUsed_Test is SDSimpleLoanSimpleProposalTest {
    function testFuzz_shouldReturnUsedCredit(uint256 used) external {
        vm.store(
            address(proposalContract), keccak256(abi.encode(_proposalHash(proposal), CREDIT_USED_SLOT)), bytes32(used)
        );
...
contract SDSimpleLoanSimpleProposal_RevokeNonce_Test is SDSimpleLoanSimpleProposalTest {
    function testFuzz_shouldCallRevokeNonce(address caller, uint256 nonceSpace, uint256 nonce) external {
        vm.expectCall(
            revokedNonce, abi.encodeWithSignature("revokeNonce(address,uint256,uint256)", caller, nonceSpace, nonce)
        );
...
contract SDSimpleLoanSimpleProposal_GetProposalHash_Test is SDSimpleLoanSimpleProposalTest {
...
contract SDSimpleLoanSimpleProposal_MakeProposal_Test is SDSimpleLoanSimpleProposalTest {
    function testFuzz_shouldFail_whenCallerIsNotProposer(address caller) external {
        vm.assume(caller != proposal.proposer);

        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoanProposal.CallerIsNotStatedProposer.selector, proposal.proposer)
        );
        vm.prank(caller);
...
contract SDSimpleLoanSimpleProposal_EncodeProposalData_Test is SDSimpleLoanSimpleProposalTest {
...
contract SDSimpleLoanSimpleProposal_DecodeProposalData_Test is SDSimpleLoanSimpleProposalTest {
    function test_shouldReturnDecodedProposalData() external {
        SDSimpleLoanSimpleProposal.Proposal memory _proposal = proposalContract.decodeProposalData(abi.encode(proposal));
...
contract SDSimpleLoanSimpleProposal_AcceptProposal_Test is
    SDSimpleLoanSimpleProposalTest,
    SDSimpleLoanProposal_AcceptProposal_Test
{
    function setUp() public virtual override(SDSimpleLoanSimpleProposalTest, SDSimpleLoanProposalTest) {
        super.setUp();
    }
    ...
    function testFuzz_shouldReturnProposalHashAndLoanTerms(bool isOffer) external {
        ...
        (bytes32 proposalHash, SDSimpleLoan.Terms memory terms) = proposalContract.acceptProposal({
            acceptor: acceptor,
            refinancingLoanId: 0,
            proposalData: abi.encode(proposal),
