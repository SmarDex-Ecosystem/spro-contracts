...
abstract contract SDSimpleLoanTest is Test {
    ...
    uint256 public constant TOKEN_FACTOR = 42e19;
    ...
    SDSimpleLoan.LOAN simpleLoan;
    SDSimpleLoan.LOAN nonExistingLoan;
    SDSimpleLoan.Terms simpleLoanTerms;
    SDSimpleLoan.ProposalSpec proposalSpec;
    SDSimpleLoan.LenderSpec lenderSpec;
    SDSimpleLoan.CallerSpec callerSpec;
    SDSimpleLoan.ExtensionProposal extension;
    T20 fungibleAsset;
    ...
    event ExtensionProposalMade(
        bytes32 indexed extensionHash, address indexed proposer, SDSimpleLoan.ExtensionProposal proposal
    );
    ...
    function setUp() public virtual {
        ...
        vm.etch(sink, bytes("data"));

        loan = new SDSimpleLoan(hub, loanToken, config, revokedNonce, categoryRegistry);
        ...
        simpleLoanTerms = SDSimpleLoan.Terms({
            lender: lender,
            borrower: borrower,
        ...
        nonExistingLoan = SDSimpleLoan.LOAN({
            status: 0,
        ...
        extension = SDSimpleLoan.ExtensionProposal({
            loanId: loanId,
        ...
        vm.mockCall(
            categoryRegistry, abi.encodeWithSignature("registeredCategoryValue(address)"), abi.encode(type(uint8).max)
        );

        vm.mockCall(config, abi.encodeWithSignature("unlistedFee()"), abi.encode(UNLISTED_FEE));
        vm.mockCall(config, abi.encodeWithSignature("listedFee()"), abi.encode(LISTED_FEE));
        vm.mockCall(config, abi.encodeWithSignature("variableFactor()"), abi.encode(VARIABLE_FACTOR));
        vm.mockCall(config, abi.encodeWithSignature("tokenFactors(address)"), abi.encode(0));

        vm.mockCall(config, abi.encodeWithSignature("sdex()"), abi.encode(address(sdex)));
        vm.mockCall(config, abi.encodeWithSignature("sink()"), abi.encode(sink));
        ...
        _mockLoanTerms(simpleLoanTerms, false);
    ...
    function _assertLOANEq(SDSimpleLoan.LOAN memory _simpleLoan1, SDSimpleLoan.LOAN memory _simpleLoan2)
        internal
        pure
    {
    ...
    function _assertLOANEq(uint256 _loanId, SDSimpleLoan.LOAN memory _simpleLoan) internal view {
        ...
        _assertLOANWord(
            loanSlot + 1,
            abi.encodePacked(
                uint16(0), _simpleLoan.defaultTimestamp, _simpleLoan.startTimestamp, _simpleLoan.originalSourceOfFunds
            )
        );
        // Borrower address
        ...
        _assertLOANWord(
            loanSlot + 3, abi.encodePacked(uint72(0), _simpleLoan.accruingInterestAPR, _simpleLoan.originalLender)
        );
        ...
        _assertLOANWord(
            loanSlot + 6,
            abi.encodePacked(uint88(0), _simpleLoan.collateral.assetAddress, _simpleLoan.collateral.category)
        );
        // Collateral id
    ...
    function _mockLOAN(uint256 _loanId, SDSimpleLoan.LOAN memory _simpleLoan) internal {
        uint256 loanSlot = uint256(keccak256(abi.encode(_loanId, LOANS_SLOT)));
    ...
    function _mockLoanTerms(SDSimpleLoan.Terms memory _terms, bool isListed) internal {
        sdex.mint(_terms.lender, type(uint128).max);
        vm.prank(_terms.lender);
        sdex.approve(address(loan), type(uint256).max);
        ...
        if (isListed) {
            vm.mockCall(
                config,
                abi.encodeWithSignature("tokenFactors(address)", _terms.credit.assetAddress),
                abi.encode(TOKEN_FACTOR)
            );
        }
    }
    ...
    function _mockExtensionProposalMade(SDSimpleLoan.ExtensionProposal memory _extension) internal {
    ...
    function _assertLOANWord(uint256 wordSlot, bytes memory word) private view {
        assertEq(abi.encodePacked(vm.load(address(loan), bytes32(wordSlot))), word);
    }
    ...
    function _extensionHash(SDSimpleLoan.ExtensionProposal memory _extension) internal view returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                keccak256(
...
contract SDSimpleLoan_GetLenderSpecHash_Test is SDSimpleLoanTest {
    function test_shouldReturnLenderSpecHash() external view {
...
contract SDSimpleLoan_CreateLOAN_Test is SDSimpleLoanTest {
    function testFuzz_shouldFail_whenProposalContractNotTagged_LOAN_PROPOSAL(address _proposalContract) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(AddressMissingHubTag.selector, _proposalContract, PWNHubTags.LOAN_PROPOSAL)
        );
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldRevokeCallersNonce_whenFlagIsTrue(address caller, uint256 nonce) external {
        ...
        vm.expectCall(revokedNonce, abi.encodeWithSignature("revokeNonce(address,uint256)", caller, nonce));
    ...
    function testFuzz_shouldCallProposalContract(
        ...
        bytes32[] memory _proposalInclusionProof,
    ...
    function testFuzz_shouldFail_whenCallerNotLender_whenLenderSpecHashMismatch(bytes32 lenderSpecHash) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoan.InvalidLenderSpecHash.selector, lenderSpecHash, correctLenderSpecHash)
        );
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldFail_whenLoanTermsDurationLessThanMin(uint256 duration) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidDuration.selector, duration, minDuration));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldFail_whenLoanTermsInterestAPROutOfBounds(uint256 interestAPR) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InterestAPROutOfBounds.selector, interestAPR, maxInterest));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function test_shouldTransferCollateral_fromBorrower_toVault_unlistedFeeToSink() external {
        ...
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
        assertEq(sdex.balanceOf(sink), UNLISTED_FEE);
    }

    function test_shouldTransferCollateral_fromBorrower_toVault_listedFeeToSink() external {
        simpleLoanTerms.collateral.category = MultiToken.Category.ERC20;
        simpleLoanTerms.collateral.assetAddress = address(fungibleAsset);
        simpleLoanTerms.collateral.id = 0;
        simpleLoanTerms.collateral.amount = 100;
        _mockLoanTerms(simpleLoanTerms, true);
        ...
        uint256 expectedFee =
            LISTED_FEE + (((VARIABLE_FACTOR * TOKEN_FACTOR) / 1e18) * simpleLoanTerms.credit.amount) / 1e18;
        assertEq(sdex.balanceOf(sink), expectedFee);
    }
    ...
    function test_shouldFail_whenPoolAdapterNotRegistered_whenPoolSourceOfFunds() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidSourceOfFunds.selector, sourceOfFunds));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldTransferCredit_toBorrower_UnlistedFeeToSink(uint256 unlistedFee, uint256 loanAmount)
        external
    {
        unlistedFee = bound(unlistedFee, 0, 1e26);
        loanAmount = bound(loanAmount, 1, 1e40);
        ...
        vm.expectCall({
            callee: address(sdex),
            data: abi.encodeWithSignature("transferFrom(address,address,uint256)", lender, sink, UNLISTED_FEE),
            count: UNLISTED_FEE > 0 ? 1 : 0
        });
    ...
    function testFuzz_shouldTransferCredit_toBorrower_ListedFeeToSink(
        uint256 listedFee,
        uint256 variableFactor,
        uint256 tokenFactor,
        uint256 loanAmount
    ) external {
        listedFee = bound(listedFee, 1, 1e26);
        variableFactor = bound(variableFactor, 1e13, 1e26);
        tokenFactor = bound(tokenFactor, 1, 1e26);
        loanAmount = bound(loanAmount, 1, 1e40);

        vm.mockCall(config, abi.encodeWithSignature("listedFee()"), abi.encode(listedFee));
        vm.mockCall(config, abi.encodeWithSignature("variableFactor()"), abi.encode(variableFactor));
        vm.mockCall(config, abi.encodeWithSignature("tokenFactors(address)"), abi.encode(tokenFactor));
        ...
        uint256 feeAmount = listedFee + (((variableFactor * tokenFactor) / 1e18) * simpleLoanTerms.credit.amount) / 1e18;

        // Mint fee amount and approve loan contract
        sdex.mint(simpleLoanTerms.lender, feeAmount);
        vm.prank(simpleLoanTerms.lender);
        sdex.approve(address(loan), type(uint256).max);

        // Fee transfer
        vm.expectCall({
            callee: address(sdex),
            data: abi.encodeWithSignature("transferFrom(address,address,uint256)", lender, sink, feeAmount),
        ...
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
        assertEq(sdex.balanceOf(sink), feeAmount);
    }
    ...
    function test_shouldEmit_LOANCreated() external {
        ...
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: "lil extra"});
    }

    function testFuzz_shouldReturnNewLoanId(uint256 _loanId) external {
        _mockLOANMint(_loanId);

        uint256 createdLoanId =
            loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
...
contract SDSimpleLoan_RefinanceLOAN_Test is SDSimpleLoanTest {
    SDSimpleLoan.LOAN refinancedLoan;
    SDSimpleLoan.Terms refinancedLoanTerms;
    ...
    function setUp() public override {
        ...
        refinancedLoan = SDSimpleLoan.LOAN({
            status: 2,
            creditAddress: address(fungibleAsset),
            originalSourceOfFunds: lender,
            startTimestamp: uint40(block.timestamp),
        ...
        refinancedLoanTerms = SDSimpleLoan.Terms({
            lender: lender,
            borrower: borrower,
            duration: 40039,
            collateral: MultiToken.ERC721(address(nonFungibleAsset), 2),
        ...
        _mockLoanTerms(refinancedLoanTerms, false);
    ...
    function test_shouldFail_whenLoanDoesNotExist() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.NonExistingLoan.selector));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function test_shouldFail_whenLoanIsNotRunning() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.LoanNotRunning.selector));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function test_shouldFail_whenLoanIsDefaulted() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.LoanDefaulted.selector, simpleLoan.defaultTimestamp));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldFail_whenCreditAssetMismatch(address _assetAddress) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.RefinanceCreditMismatch.selector));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldFail_whenCollateralCategoryMismatch(uint8 _category) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.RefinanceCollateralMismatch.selector));
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function testFuzz_shouldFail_whenBorrowerMismatch(address _borrower) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoan.RefinanceBorrowerMismatch.selector, simpleLoan.borrower, _borrower)
        );
        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});
    }
    ...
    function test_shouldEmit_LOANCreated() external {
        vm.expectEmit();
        emit LOANCreated(
            loanId, proposalHash, proposalContract, refinancingLoanId, refinancedLoanTerms, lenderSpec, "lil extra"
        );
    ...
    function test_shouldWithdrawCreditWithoutCommon_whenShouldNotTransferCommon_whenPoolSourceOfFunds() external {
        ...
        vm.expectCall(
            ...
            abi.encodeWithSignature(
                ...
                newLender,
                refinancedLoanTerms.credit.assetAddress,
                refinancedLoanTerms.credit.amount - common
            )
        );
    ...
    function test_shouldNotWithdrawCredit_whenShouldNotTransferCommon_whenNoSurplus_whenNoFee_whenPoolSourceOfFunds()
        external
    {
        lenderSpec.sourceOfFunds = sourceOfFunds;
        refinancedLoanTerms.lender = newLender;
    ...
    function testFuzz_shouldTransferFeeToCollector(uint256 unlistedFee) external {
        unlistedFee = bound(unlistedFee, 1, type(uint128).max);
        sdex.mint(refinancedLoanTerms.lender, unlistedFee);
        vm.prank(refinancedLoanTerms.lender);
        sdex.approve(address(loan), type(uint256).max);

        vm.mockCall(config, abi.encodeWithSignature("unlistedFee()"), abi.encode(unlistedFee));

        vm.expectCall(
            address(sdex), abi.encodeWithSignature("transferFrom(address,address,uint256)", lender, sink, unlistedFee)
        );
    ...
    function test_shouldTransferCommonToVaul_whenLenderNotLoanOwner() external {
        ...
        uint256 common = Math.min(refinancedLoanTerms.credit.amount, loan.loanRepaymentAmount(refinancingLoanId));
    ...
    function testFuzz_shouldTransferCommonToVault_whenLenderOriginalLender_whenDifferentSourceOfFunds() external {
        lenderSpec.sourceOfFunds = newLender;
        refinancedLoanTerms.lender = newLender;
        refinancedLoanTerms.lenderSpecHash = loan.getLenderSpecHash(lenderSpec);
        _mockLoanTerms(refinancedLoanTerms, false);
    ...
    function testFuzz_shouldNotTransferCommonToVaul_whenLenderLoanOwner_whenLenderOriginalLender_whenSameSourceOfFunds(
        bool sourceOfFundsflag
    ) external {
    ...
    function test_shouldNotTransferShortageFromBorrowerToVault_whenNoShortage() external {
        refinancedLoanTerms.credit.amount = loan.loanRepaymentAmount(refinancingLoanId);
        _mockLoanTerms(refinancedLoanTerms, false);
    ...
    function testFuzz_shouldTryClaimRepaidLOAN_shortageAmount_whenShouldNotTransferCommon(uint256 shortage) external {
        ...
        vm.expectCall(
            address(loan),
            abi.encodeWithSignature("tryClaimRepaidLOAN(uint256,uint256,address)", refinancingLoanId, shortage, lender)
        );
    ...
    function test_shouldNotFail_whenTryClaimRepaidLOANFails() external {
        vm.mockCallRevert(address(loan), abi.encodeWithSignature("tryClaimRepaidLOAN(uint256,uint256,address)"), "");
    ...
    function testFuzz_shouldRepayOriginalLoan(
        ...
        refinanceAmount =
            bound(refinanceAmount, 1, type(uint256).max - loanRepaymentAmount - fungibleAsset.totalSupply());
    ...
    function testFuzz_shouldCollectProtocolFee(
        ...
        unlistedFee = bound(unlistedFee, 1, 1e26);
        vm.mockCall(config, abi.encodeWithSignature("unlistedFee()"), abi.encode(unlistedFee));
        ...
        vm.mockCall(
            proposalContract,
            abi.encodeWithSignature("acceptProposal(address,uint256,bytes,bytes32[],bytes)"),
            abi.encode(proposalHash, refinancedLoanTerms)
        );
        ...
        uint256 originalBalance = sdex.balanceOf(sink);

        loan.createLOAN({proposalSpec: proposalSpec, lenderSpec: lenderSpec, callerSpec: callerSpec, extra: ""});

        assertEq(sdex.balanceOf(sink), originalBalance + unlistedFee);
    }
    ...
    function testFuzz_shouldTransferSurplusToBorrower(uint256 refinanceAmount) external {
        ...
        refinanceAmount = bound(
            ...
            type(uint256).max - loanRepaymentAmount - fungibleAsset.totalSupply()
        );
...
contract SDSimpleLoan_RepayLOAN_Test is SDSimpleLoanTest {
...
contract SDSimpleLoan_LoanRepaymentAmount_Test is SDSimpleLoanTest {
    function test_shouldReturnZero_whenLoanDoesNotExist() external view {
...
contract SDSimpleLoan_ClaimLOAN_Test is SDSimpleLoanTest {
    function setUp() public override {
        super.setUp();
    ...
    function testFuzz_shouldFail_whenCallerIsNotLOANTokenHolder(address caller) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.CallerNotLOANTokenHolder.selector));
        vm.prank(caller);
    ...
    function test_shouldFail_whenLoanIsNotRepaidNorExpired() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.LoanRunning.selector));
        vm.prank(lender);
        loan.claimLOAN(loanId);
    }
    ...
    function testFuzz_shouldTransferRepaidAmountToLender_whenLoanIsRepaid(uint256 _principal, uint256 _fixedInterest)
        external
    {
        _principal = bound(_principal, 1, 1e40);
        _fixedInterest = bound(_fixedInterest, 0, 1e40);
        ...
        vm.expectCall(
            simpleLoan.creditAddress, abi.encodeWithSignature("transfer(address,uint256)", lender, loanRepaymentAmount)
        );
    ...
    function test_shouldTransferCollateralToLender_whenLoanIsDefaulted() external {
        ...
        vm.expectCall(
            simpleLoan.collateral.assetAddress,
            abi.encodeWithSignature(
                "safeTransferFrom(address,address,uint256,bytes)", address(loan), lender, simpleLoan.collateral.id, ""
            )
        );
...
contract SDSimpleLoan_TryClaimRepaidLOAN_Test is SDSimpleLoanTest {
    ...
    function testFuzz_shouldFail_whenCallerIsNotVault(address caller) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.CallerNotVault.selector));
        vm.prank(caller);
    ...
    function test_shouldTransferToOriginalLender_whenSourceOfFundsEqualToOriginalLender() external {
        ...
        vm.expectCall(
            simpleLoan.creditAddress, abi.encodeWithSignature("transfer(address,uint256)", lender, creditAmount)
        );
    ...
    function test_shouldTransferAmountToPoolAdapter_whenSourceOfFundsNotEqualToOriginalLender() external {
        ...
        vm.expectCall(
            simpleLoan.creditAddress, abi.encodeWithSignature("transfer(address,uint256)", poolAdapter, creditAmount)
        );
...
contract SDSimpleLoan_MakeExtensionProposal_Test is SDSimpleLoanTest {
    function testFuzz_shouldFail_whenCallerNotProposer(address caller) external {
        vm.assume(caller != extension.proposer);

        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoan.InvalidExtensionSigner.selector, extension.proposer, caller)
        );
        vm.prank(caller);
...
contract SDSimpleLoan_ExtendLOAN_Test is SDSimpleLoanTest {
    uint256 lenderPk;
    uint256 borrowerPk;
    ...
    function _signExtension(uint256 pk, SDSimpleLoan.ExtensionProposal memory _extension)
        private
        view
    ...
    function test_shouldFail_whenLoanIsRepaid() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.LoanRepaid.selector));
        vm.prank(lender);
        loan.extendLOAN(extension, "", "");
    }
    ...
    function testFuzz_shouldFail_whenInvalidSignature_whenEOA(uint256 pk) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PWNSignatureChecker.InvalidSignature.selector, extension.proposer, _extensionHash(extension)
            )
        );
        vm.prank(lender);
    ...
    function test_shouldFail_whenOfferNonceNotUsable() external {
        ...
        vm.mockCall(
            revokedNonce,
            abi.encodeWithSignature(
                "isNonceUsable(address,uint256,uint256)", extension.proposer, extension.nonceSpace, extension.nonce
            ),
            abi.encode(false)
        );
    ...
    function testFuzz_shouldFail_whenCallerIsNotBorrowerNorLoanOwner(address caller) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidExtensionCaller.selector));
        vm.prank(caller);
        loan.extendLOAN(extension, "", "");
    }
    ...
    function testFuzz_shouldFail_whenCallerIsBorrower_andProposerIsNotLoanOwner(address proposer) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidExtensionSigner.selector, lender, proposer));
        vm.prank(borrower);
        loan.extendLOAN(extension, "", "");
    }
    ...
    function testFuzz_shouldFail_whenCallerIsLoanOwner_andProposerIsNotBorrower(address proposer) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidExtensionSigner.selector, borrower, proposer));
        vm.prank(lender);
        loan.extendLOAN(extension, "", "");
    }
    ...
    function testFuzz_shouldFail_whenExtensionDurationLessThanMin(uint40 duration) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidExtensionDuration.selector, duration, minDuration));
        vm.prank(lender);
        loan.extendLOAN(extension, "", "");
    }
    ...
    function testFuzz_shouldFail_whenExtensionDurationMoreThanMax(uint40 duration) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoan.InvalidExtensionDuration.selector, duration, maxDuration));
        vm.prank(lender);
        loan.extendLOAN(extension, "", "");
    }
    ...
    function test_shouldNotTransferCredit_whenAddressZero() external {
        ...
        vm.expectCall({
            ...
            data: abi.encodeWithSignature(
                "transferFrom(address,address,uint256)", borrower, lender, extension.compensationAmount
            ),
            count: 0
        });
...
contract SDSimpleLoan_GetExtensionHash_Test is SDSimpleLoanTest {
    function test_shouldReturnExtensionHash() external view {
...
contract SDSimpleLoan_GetLOAN_Test is SDSimpleLoanTest {
    function testFuzz_shouldReturnStaticLOANData_FirstPart(
        ...
        {
            (,,, address borrower_,,,,,,,,) = loan.getLOAN(loanId);
            assertEq(borrower_, _borrower);
        }
        {
    ...
    function testFuzz_shouldReturnStaticLOANData_SecondPart(address _originalSourceOfFunds) external {
    ...
    function test_shouldReturnEmptyLOANDataForNonExistingLoan() external view {
...
contract SDSimpleLoan_LoanMetadataUri_Test is SDSimpleLoanTest {
    string tokenUri;
    ...
    function setUp() public override {
        ...
        vm.mockCall(config, abi.encodeWithSignature("loanMetadataUri(address)"), abi.encode(tokenUri));
    }

    function test_shouldCallConfig() external {
        vm.expectCall(config, abi.encodeWithSignature("loanMetadataUri(address)", loan));
...
contract SDSimpleLoan_GetStateFingerprint_Test is SDSimpleLoanTest {
    ...
    function test_shouldUpdateStateFingerprint_whenLoanDefaulted() external {
        ...
        assertEq(
            ...
            keccak256(
                abi.encode(
                    2, simpleLoan.defaultTimestamp, simpleLoan.fixedInterestAmount, simpleLoan.accruingInterestAPR
                )
            )
        );
        ...
        assertEq(
            ...
            keccak256(
                abi.encode(
                    4, simpleLoan.defaultTimestamp, simpleLoan.fixedInterestAmount, simpleLoan.accruingInterestAPR
                )
            )
        );
    }

    function testFuzz_shouldReturnCorrectStateFingerprint(uint256 fixedInterestAmount, uint24 accruingInterestAPR)
        external
    {
