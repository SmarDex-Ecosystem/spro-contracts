...
abstract contract SDSimpleLoanProposalTest is Test {
    ...
    SDSimpleLoanProposal public proposalContractAddr; // Need to set in the inheriting contract

    struct Params {
        SDSimpleLoanProposal.ProposalBase base;
        address acceptor;
    ...
    function _callAcceptProposalWith() internal returns (bytes32, SDSimpleLoan.Terms memory) {
    ...
    function _callAcceptProposalWith(Params memory _params)
        internal
        virtual
        returns (bytes32, SDSimpleLoan.Terms memory);
...
abstract contract SDSimpleLoanProposal_AcceptProposal_Test is SDSimpleLoanProposalTest {
    function testFuzz_shouldFail_whenCallerIsNotProposedLoanContract(address caller) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoanProposal.CallerNotLoanContract.selector, caller, activeLoanContract)
        );
        vm.prank(caller);
    ...
    function testFuzz_shouldFail_withInvalidSignature_whenEOA_whenMultiproposal(uint256 randomPK) external {
        ...
        bytes32 multiproposalHash = proposalContractAddr.getMultiproposalHash(SDSimpleLoanProposal.Multiproposal(root));
        params.signature = _sign(randomPK, multiproposalHash);
        params.proposalInclusionProof = proposalInclusionProof;

        vm.expectRevert(
            abi.encodeWithSelector(PWNSignatureChecker.InvalidSignature.selector, proposer, multiproposalHash)
        );
        vm.prank(activeLoanContract);
        _callAcceptProposalWith();
    }
    ...
    function test_shouldFail_withInvalidInclusionProof() external {
        ...
        bytes32 root = keccak256(
            uint256(proposalHash) < uint256(leaf) ? abi.encode(proposalHash, leaf) : abi.encode(leaf, proposalHash)
        );
        bytes32 multiproposalHash = proposalContractAddr.getMultiproposalHash(SDSimpleLoanProposal.Multiproposal(root));
        ...
        bytes32 actualMultiproposalHash =
            proposalContractAddr.getMultiproposalHash(SDSimpleLoanProposal.Multiproposal(actualRoot));
        vm.expectRevert(
    ...
    function test_shouldFail_whenProposerIsSameAsAcceptor() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(SDSimpleLoanProposal.AcceptorIsProposer.selector, proposer));
        vm.prank(activeLoanContract);
        _callAcceptProposalWith();
    }

    function testFuzz_shouldFail_whenProposedRefinancingLoanIdNotZero_whenRefinancingLoanIdZero(
        uint256 proposedRefinancingLoanId
    ) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoanProposal.InvalidRefinancingLoanId.selector, proposedRefinancingLoanId)
        );
        vm.prank(activeLoanContract);
        _callAcceptProposalWith();
    }
    ...
    function testFuzz_shouldFail_whenOfferNonceNotUsable(uint256 nonceSpace, uint256 nonce) external {
        ...
        vm.mockCall(revokedNonce, abi.encodeWithSignature("isNonceUsable(address,uint256,uint256)"), abi.encode(false));
        vm.expectCall(
            revokedNonce, abi.encodeWithSignature("isNonceUsable(address,uint256,uint256)", proposer, nonceSpace, nonce)
        );
    ...
    function testFuzz_shouldFail_whenCallerIsNotAllowedAcceptor(address caller) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoanProposal.CallerNotAllowedAcceptor.selector, caller, allowedAcceptor)
        );
        vm.prank(activeLoanContract);
        _callAcceptProposalWith();
    }
    ...
    function test_shouldRevokeOffer_whenAvailableCreditLimitEqualToZero(uint256 nonceSpace, uint256 nonce) external {
        ...
        vm.expectCall(
            revokedNonce, abi.encodeWithSignature("revokeNonce(address,uint256,uint256)", proposer, nonceSpace, nonce)
        );
    ...
    function testFuzz_shouldFail_whenUsedCreditExceedsAvailableCreditLimit(uint256 used, uint256 limit) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SDSimpleLoanProposal.AvailableCreditLimitExceeded.selector, used + params.base.creditAmount, limit
            )
        );
        vm.prank(activeLoanContract);
        _callAcceptProposalWith();
    ...
    modifier whenBelowAvailableCreditLimit() {
        _;
    }

    function testFuzz_shouldFail_whenCreditAmountLtMinimumAmount(uint256 amount, uint256 used, uint256 limit)
        external
        whenBelowAvailableCreditLimit
    {
        limit = bound(limit, 100e18, 1_000_000_000_000e18);

        uint256 minimumCreditAmount = (500 * limit) / 1e4;
        uint256 upperThreshold = (9500 * limit) / 1e4;

        used = bound(used, minimumCreditAmount, upperThreshold);
        amount = bound(amount, 1, minimumCreditAmount - 1); // ensures creditAmount lt minimumCreditAmount

        params.base.availableCreditLimit = limit;
        params.base.creditAmount = amount;
        params.signature = _sign(proposerPK, _getProposalHashWith());

        bytes32 proposalHash = _getProposalHashWith();

        vm.store(address(proposalContractAddr), keccak256(abi.encode(proposalHash, CREDIT_USED_SLOT)), bytes32(used));

        vm.expectRevert(
            abi.encodeWithSelector(SDSimpleLoanProposal.CreditAmountTooSmall.selector, amount, minimumCreditAmount)
        );
        vm.prank(activeLoanContract);
        _callAcceptProposalWith();
    }

    modifier whenAboveMimumumCreditAmount() {
        _;
    }

    function testFuzz_shouldIncreaseUsedCredit(uint256 used, uint256 limit)
        external
        whenBelowAvailableCreditLimit
        whenAboveMimumumCreditAmount
    {
        limit = bound(limit, 100e18, 1_000_000_000_000e18);
        uint256 minimumCreditAmount = (500 * limit) / 1e4;
        uint256 upperThreshold = (9500 * limit) / 1e4;
        ...
        params.base.availableCreditLimit = limit;
        params.base.creditAmount = minimumCreditAmount;
        params.signature = _sign(proposerPK, _getProposalHashWith());
    ...
    modifier whenCreditUsedExceedsUpperThresholdLimit() {
        _;
    }

    function testFuzz_shouldIncreaseUsedCreditAndRevokeNonce(uint256 limit, uint256 nonceSpace, uint256 nonce)
        external
        whenBelowAvailableCreditLimit
        whenAboveMimumumCreditAmount
        whenCreditUsedExceedsUpperThresholdLimit
    {
        limit = bound(limit, 100e18, 1_000_000_000_000e18);
        uint256 minimumCreditAmount = (500 * limit) / 1e4;
        uint256 used = (9500 * limit) / 1e4;
    ...
    function testFuzz_shouldFail_whenNonceRevokedAfterCreditUsedAboveThreshold(
        uint256 limit,
        uint256 nonceSpace,
        uint256 nonce
    ) external whenBelowAvailableCreditLimit whenAboveMimumumCreditAmount whenCreditUsedExceedsUpperThresholdLimit {
        limit = bound(limit, 100e18, 1_000_000_000_000e18);
        uint256 minimumCreditAmount = (500 * limit) / 1e4;
        uint256 used = (9500 * limit) / 1e4 - (minimumCreditAmount / 2);
        ...
        // Mock call: nonce should not be callable now
        vm.mockCall(revokedNonce, abi.encodeWithSignature("isNonceUsable(address,uint256,uint256)"), abi.encode(false));
        ...
        vm.store(
            ...
            bytes32(used + minimumCreditAmount)
        );
    ...
    function test_shouldNotCallComputerRegistry_whenShouldNotCheckStateFingerprint() external {
        ...
        vm.expectCall({callee: config, data: abi.encodeWithSignature("getStateFingerprintComputer(address)"), count: 0});
    ...
    function test_shouldCallComputerRegistry_whenShouldCheckStateFingerprint() external {
        ...
        vm.expectCall(
            config, abi.encodeWithSignature("getStateFingerprintComputer(address)", params.base.collateralAddress)
        );
    ...
    function test_shouldFail_whenComputerRegistryReturnsComputer_whenComputerFails() external {
        ...
        vm.mockCallRevert(
            stateFingerprintComputer, abi.encodeWithSignature("computeStateFingerprint(address,uint256)"), "some error"
        );
    ...
    function testFuzz_shouldFail_whenComputerRegistryReturnsComputer_whenComputerReturnsDifferentStateFingerprint(
        ...
        vm.mockCall(
            ...
            abi.encodeWithSignature(
                "computeStateFingerprint(address,uint256)", params.base.collateralAddress, params.base.collateralId
            ),
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SDSimpleLoanProposal.InvalidCollateralStateFingerprint.selector,
                stateFingerprint,
    ...
    function test_shouldFail_whenNoComputerRegistered_whenAssetDoesNotImplementERC165() external {
        ...
        vm.mockCall(config, abi.encodeWithSignature("getStateFingerprintComputer(address)"), abi.encode(address(0)));
        vm.mockCallRevert(
